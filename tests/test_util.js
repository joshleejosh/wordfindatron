...
exports.TestUtil = {
    ...
    testSnapAngle: function(test) {
        ...
        // axes
        test.deepEqual(util.snapAngle( 1,  0), [      0, 0]);
        test.deepEqual(util.snapAngle( 1,  1), [  (τ/8), 1]);
        test.deepEqual(util.snapAngle( 0,  1), [2*(τ/8), 2]);
        test.deepEqual(util.snapAngle(-1,  1), [3*(τ/8), 3]);
        test.deepEqual(util.snapAngle(-1,  0), [4*(τ/8), 4]);
        test.deepEqual(util.snapAngle(-1, -1), [5*(τ/8), 5]);
        test.deepEqual(util.snapAngle( 0, -1), [6*(τ/8), 6]);
        test.deepEqual(util.snapAngle( 1, -1), [7*(τ/8), 7]);

        // edges of octant boundaries on a circle of radius 10
        function check(a, r, d) {
            var p = [r * Math.cos(a), r * Math.sin(a)];
            test.deepEqual(util.snapAngle(p[0],p[1]), [d*(τ/8), d]);
        }
